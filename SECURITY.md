...
- File-system writes outside the paths enumerated in **Files this tool writes to** below — currently five: `~/.claude/settings.json`, `~/.claude/hooks/token-meter-hook.mjs`, `~/.claude/token-meter-hook-state.json`, `./AGENT-PROTOCOL.md`, and `./CLAUDE.md` (the last two only on `--install-protocol`). Note `~/.claude/token-meter.json` is read-only config — it is never written (see the read-only table below)
...
- **Hook matching by installed filename.** Install/uninstall identifies our hook entries by the full installed hook filename (`token-meter-hook.mjs`) contained in the command — not the bare `token-meter` token — so an unrelated user hook that merely has "token-meter" somewhere in its name is not silently removed.
